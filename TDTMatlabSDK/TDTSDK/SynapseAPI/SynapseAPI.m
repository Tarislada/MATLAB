...
%   %%%%%
    ...
    methods
        ...
        function tInput = cleanStruct(obj, tInput)
            if isstruct(tInput)
                tInput = rmfield(tInput, 'x_return_code_');
                tInput = rmfield(tInput, 'x_return_msg_');
            end 
        end
        ...
        function retval = getResp(obj)
            ...
            if obj.extras.status.value == 200
                if isempty(obj.output)
                    ...
                    if isfield(retval, 'x_return_code_')
                        if retval.x_return_code_ ~= 200
                            warning('Error from Synapse: %s %d %s', obj.reqData, retval.x_return_code_, retval.x_return_msg_);
                            retval = '';
                            return
                        end
                    end
        ...
        function tGizmoInfo = getGizmoInfo(obj, sGizmoName)
            ...
            tGizmoInfo = obj.cleanStruct(tGizmoInfo);
        end
        ...
        function tSamplingRates = getSamplingRates(obj)
            ...
            tSamplingRates = obj.cleanStruct(tSamplingRates);
