...
function [tmpcell,resultmat] = bayespractice3(avescmat,divind)
...
%         row1mat = zeros(subject_num,max(minescape));
%         row2mat = zeros(subject_num,max(minescape));
        resultmat = nan(subject_num,min(maxescape));
        for i = 1:subject_num
            workvec = tmpcell{1,i};
            ...
            if length(zeroidx) < min(maxescape)
                aftescvec = [aftescvec; 1];
            end
            resultmat(i,:) = aftescvec(1:min(maxescape));
...
%         resultmat = [1:max(minescape);sum(row1mat,1);sum(row2mat,1)];
end        
